class Staff::MatchReportsController < StaffController
  ...
  def destroy
    ...
    @siapa1.update(perlawanan: perlawanan_home -= 1)
    @siapa2.update(perlawanan: perlawanan_away -= 1)
    @siapa1.update(gol_masuk: current_home_gm -= @staff_match_report.score_home_team_was)
    @siapa1.update(gol_bolos: current_home_gb -= @staff_match_report.score_away_team_was)
    @siapa2.update(gol_masuk: current_away_gm -= @staff_match_report.score_away_team_was)
    @siapa2.update(gol_bolos: current_away_gb -= @staff_match_report.score_home_team_was)
    ...
    if @staff_match_report.destroy
      redirect_to staff_zone_group_team_schedule_path(id:@staff_team_schedule), notice: 'Berjaya dihapuskan!'
    else
      render 'index'
    end
  end
